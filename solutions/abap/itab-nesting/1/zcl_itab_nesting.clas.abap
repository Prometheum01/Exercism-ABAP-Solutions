...
*Tum tablolari donuyoruz ic ice. Once artistten baslayip sonrasinda album ve sonra sarkiye girip. 
*Sarkilari formatlayip yukariya dogru cikariyoruz.

  LOOP AT artists INTO DATA(ls_artist).
    DATA ls_nested_artist TYPE artist_album_nested_type.
    CLEAR ls_nested_artist.
    
    ls_nested_artist-artist_id = ls_artist-artist_id.
    ls_nested_artist-artist_name = ls_artist-artist_name.

    LOOP AT albums INTO DATA(ls_album) WHERE artist_id = ls_artist-artist_id.
      DATA ls_nested_album TYPE album_song_nested_type.
      CLEAR ls_nested_album.
      
      ls_nested_album-album_id = ls_album-album_id.
      ls_nested_album-album_name = ls_album-album_name.

      LOOP AT songs INTO DATA(ls_song) WHERE artist_id = ls_album-artist_id AND album_id = ls_album-album_id.
        DATA ls_nested_song TYPE song_nested_type.
        CLEAR ls_nested_song.
        
        ls_nested_song-song_id = ls_song-song_id.
        ls_nested_song-song_name = ls_song-song_name.
        
        APPEND ls_nested_song TO ls_nested_album-songs.
      ENDLOOP.

      APPEND ls_nested_album TO ls_nested_artist-albums.
    ENDLOOP.

    APPEND ls_nested_artist TO nested_data.
  ENDLOOP.
